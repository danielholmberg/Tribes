...
class DatabaseService {
  ...
  Stream<UserData> currentUserDataStream({String firebaseUserID}) {
    return usersRoot
        .document(firebaseUserID ?? _currentUserData.id)
        .snapshots()
        .map((snapshot) {
          _currentUserData = UserData.fromSnapshot(snapshot);
  ...
  Future<bool> doesUserExist(String uid) async {
    DocumentSnapshot userDoc = await usersRoot.document(uid).get();
    return userDoc.exists;
  }

  Future<UserData> getUserData(String uid) {
    return usersRoot.document(uid).get().then((doc) => UserData.fromSnapshot(doc));
  }
